...
export default function WatchSection({ watch, index, onClick }) {
  ...
  // White stage layout behind the videos — the video itself stays untouched
  const cardBackdrops = [
    'bg-white', // Card 1
    'bg-white', // Card 2
    'bg-white', // Card 3
  ...
  return (
    <section
      ref={sectionRef}
      className="sticky top-0 w-full h-screen bg-white flex items-center justify-center pointer-events-auto px-3 py-3 sm:px-4 sm:py-4 md:px-3 md:py-3"
    ...
    >
      ...
      >
        {/* 1. INLINE WATCH VIDEO BACKGROUND (AUTOPLAY) — nothing layered over the video */}
        ...
        <div className="relative z-20 flex justify-between items-start w-full pointer-events-auto">
          <div>
            <span className="text-[10px] sm:text-[11px] font-mono tracking-[0.35em] text-[#C9A96E] uppercase font-bold block mb-1 drop-shadow-[0_2px_6px_rgba(0,0,0,0.9)]">
              2026 EDITION
            </span>
            <h2
              className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-white leading-tight uppercase tracking-tight drop-shadow-[0_4px_14px_rgba(0,0,0,0.95)]"
              style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}
            >
              {watch.model}
            </h2>
          ...
          <div className="flex items-center gap-4">
            <span className="text-xs font-mono tracking-[0.2em] text-white/90 font-bold uppercase drop-shadow-[0_2px_6px_rgba(0,0,0,0.9)]">
              № 0{index + 1}/04
            </span>
            {/* SOUND TOGGLE */}
            <button
              onClick={toggleAudio}
              className="px-3.5 py-2 rounded-full backdrop-blur-xl bg-black/55 border border-white/25 hover:bg-[#C9A96E]/25 hover:border-[#C9A96E]/50 transition-all duration-300 flex items-center gap-2 text-white cursor-pointer"
            >
        ...
        <div className="relative z-20 w-full flex flex-col sm:flex-row items-center justify-between gap-4 pointer-events-auto pt-6 border-t border-white/20 group-hover:border-white/40 transition-colors duration-700 mt-auto">
          <div className="text-left w-full sm:w-auto">
            <span className="text-[10px] font-mono tracking-[0.25em] text-white/85 uppercase block mb-1 drop-shadow-[0_2px_6px_rgba(0,0,0,0.9)]">
              SPECIAL PRICE
            </span>
            <span className="text-2xl sm:text-3xl font-extrabold text-[#C9A96E] tracking-tight drop-shadow-[0_3px_10px_rgba(0,0,0,0.95)]">
              {watch.price}
            </span>
          </div>
